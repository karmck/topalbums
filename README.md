...
## How It Works
- **Data source:** A Google Sheet with one tab per year (e.g., `2026`). Each tab contains a single column of album search strings (e.g., `Artist - Album`).
- **Ingestion script:** `fetch-covers.js`
  - Authenticates to Spotify via Client Credentials.
  - Reads the target year’s tab from the Google Sheet using a service account.
